...
function ChatWindow({ onClose }) {
    ...
    ws.onmessage = (e) => {
      ...
      switch (data.type) {
        ...
        case "skipped":
          setStatus("waiting");
          setStatusMsg(
            data.message || "Looking for someone else…"
          );
          break;
  ...
  const sendMessage = () => {
    ...
    messageTimestampsRef.current =
      messageTimestampsRef.current.filter(
        (timestamp) => now - timestamp < 1000
      );

    // Allow max 3 msgs/sec
    if (messageTimestampsRef.current.length >= 3) {
      setWarning(
        "⚠️ Slow down! You can send only 3 messages per second."
      );
        ...
        {status === "confirming" && (
          <div className="match-card">
            ...
            <p className="match-sub">
              Start talking or find someone else?
            </p>
